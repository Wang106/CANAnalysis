...
(async()=>{
  ...
  try{
    ...
    assert.equal(await languagePage.locator('.language-toggle').evaluate(node=>getComputedStyle(node,'::before').content),'"🌐"');
    ...
    const mobileNavBounds=await languagePage.evaluate(()=>{
      ...
      const skinStyle=getComputedStyle(skinIcon),languageStyle=getComputedStyle(languageIcon);
      return {linksRight:Math.round(linkBounds.right),switcherLeft:Math.round(switcherBounds.left),switcherTop:Math.round(switcherBounds.top),switcherWidth:Math.round(switcherBounds.width),languageWidth:Math.round(languageBounds.width),languageLeft:Math.round(languageBounds.left),languageTop:Math.round(languageIconBounds.top),languageHeight:Math.round(languageIconBounds.height),languageIconLeft:Math.round(languageIconBounds.left),skinWidth:Math.round(skinBounds.width),skinLeft:Math.round(skinBounds.left),skinTop:Math.round(skinIconBounds.top),skinHeight:Math.round(skinIconBounds.height),skinIconLeft:Math.round(skinIconBounds.left),languageOpacity:getComputedStyle(languageSelect).opacity,skinOpacity:getComputedStyle(skinSelect).opacity,skinChrome:[skinStyle.border,skinStyle.borderRadius,skinStyle.backgroundColor,skinStyle.boxSizing,skinStyle.lineHeight],languageChrome:[languageStyle.border,languageStyle.borderRadius,languageStyle.backgroundColor,languageStyle.boxSizing,languageStyle.lineHeight],lineWidth:Math.round(parseFloat(line.width)),lineColor:line.backgroundColor,linksBorder:getComputedStyle(links).borderBottomWidth};
    });
    ...
    assert.equal(mobileNavBounds.skinTop,mobileNavBounds.languageTop,'mobile icons must share the same vertical center line');
    assert.equal(mobileNavBounds.skinHeight,mobileNavBounds.languageHeight,'mobile icons must share the same control height');
    assert.deepEqual(mobileNavBounds.skinChrome,mobileNavBounds.languageChrome,'mobile skin and language icons must share identical control chrome');
    assert.equal(mobileNavBounds.skinIconLeft-mobileNavBounds.switcherLeft,3,'skin icon must occupy the first evenly distributed slot');
    assert.equal(mobileNavBounds.switcherLeft+mobileNavBounds.switcherWidth-(mobileNavBounds.languageIconLeft+mobileNavBounds.languageWidth),3,'language icon must occupy the second evenly distributed slot');

...
for(const page of sharedPages){
  ...
  assert.match(html,/<header class="can-brandbar">\s*<div class="shell can-brandbar-inner">\s*<a class="can-brand" href="\/" aria-label="返回 CANAnalysis 首页">\s*<img src="\/icon\.png" width="34" height="34" alt="">\s*<span>CAN报文解析<small>CAN DATA WORKSPACE<\/small><\/span>\s*<\/a>\s*<\/div>\s*<\/header>/s,page+' must use the shared Chinese CAN brand row');
}

const offline=fs.readFileSync('public/offline.html','utf8');
assert.match(offline,/<div class="header-left">\s*<a class="can-brand" href="\/" aria-label="返回 CANAnalysis 首页">\s*<img src="\/icon\.png" width="34" height="34" alt="">\s*<span>CAN报文解析<small>CAN DATA WORKSPACE<\/small><\/span>\s*<\/a>\s*<\/div>/s,'offline analysis must use the exact shared Chinese brand inside its working header');

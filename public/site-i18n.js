...
(() => {
  ...
  const exact = new Map(Object.entries({
    '中文':'Chinese','语言切换':'Language','本站栏目':'Site sections','栏目导航':'Site navigation','CAN报文解析':'CANAnalysis',
    ...
    '格式与兼容范围说明':'Formats & Compatibility Guide','转换的是原始 CAN 数据帧与远程帧。各工具的专有事件、注释和触发信息不在转换范围内。':'Conversion covers raw CAN data and remote frames. Tool-specific events, comments and trigger metadata are not converted.','格式':'Format','设备':'Device','说明':'Description','经典 CAN / CAN FD 文本日志':'Classic CAN / CAN FD text log','经典 CAN，ABSOLUTE / SYSTEM 时间模式':'Classic CAN, ABSOLUTE / SYSTEM time modes','1.0、1.1、1.3、2.0、2.1，经典 CAN / CAN FD':'1.0, 1.1, 1.3, 2.0 and 2.1; Classic CAN / CAN FD','未压缩 / zlib 日志容器，经典 CAN / CAN FD':'Uncompressed / zlib log container, Classic CAN / CAN FD','周立功':'ZLG','含表头的 CANTest / ZCAN 表格；支持选择编码与时间单位':'CANTest / ZCAN table with header; configurable encoding and time unit','4.00–4.11 原始 CAN 帧；未压缩、Deflate、转置 Deflate':'4.00–4.11 raw CAN frames; uncompressed, Deflate and transposed Deflate','3.x 中带 CAN_DataFrame 字段的原始帧':'3.x raw frames with CAN_DataFrame fields','DBC 信号':'DBC Signals','DBC + 上述任一 CAN 日志，100–1000 ms 固定间隔导出信号宽表':'DBC plus any supported CAN log; exports a signal wide table at fixed 100–1000 ms intervals',
    ...
    '文件留在你的设备中':'Your Files Stay on Your Device','DBC 与 ASC 的读取、解析和绘图均在当前浏览器本地完成，本站不会主动上传你的 CAN 数据文件。':'DBC and ASC reading, parsing and charting happen locally in your browser. This site does not upload your CAN data files.','支持这个小工具':'Support This Tool','如果它帮你节省了排查时间，可以自愿支持后续维护。支持与否不会影响任何功能的使用。':'If this tool saves you troubleshooting time, you may support its continued maintenance. Every feature remains available either way.','如在使用过程中遇到问题，或有功能建议与其他需求，欢迎发送邮件联系：':'If you encounter a problem or have a feature request, please contact:','海洋伙伴':'Ocean Friends','移动鼠标，附近的动物会慢慢靠近你':'Move the pointer and nearby animals will gently follow','微信':'WeChat Pay','微信扫一扫支持':'Scan with WeChat','微信二维码预留位':'WeChat QR Placeholder','支付宝':'Alipay','支付宝扫一扫支持':'Scan with Alipay','支付宝二维码预留位':'Alipay QR Placeholder','建议使用清晰的正方形二维码图片':'Use a clear square QR-code image','请在付款前核对收款方信息。二维码图片将在后续版本中补充。':'Verify the recipient before paying. QR-code images will be added in a future release.','请在付款前核对收款方信息。':'Verify the recipient before paying.','微信收款二维码':'WeChat payment QR code','支付宝收款二维码':'Alipay payment QR code','为更高效的 CAN 数据排查而设计':'Designed for more efficient CAN data troubleshooting','返回 CANAnalysis 首页':'Return to CANAnalysis home','微信收款二维码图片预留位置':'Reserved location for WeChat payment QR code','支付宝收款二维码图片预留位置':'Reserved location for Alipay payment QR code',
    '选择适合你的':'Choose the right','CAN 数据工具':'CAN data tool','从设备实时连接到离线日志分析，选择与你当前任务最匹配的入口。':'From live hardware connections to offline log analysis, choose the entry that best matches your task.','本站全部页面':'All site pages','数据优先在本地处理':'Data is processed locally first','可使用':'Available','开发中':'In development','进入在线连接':'Open Online Connection','进入离线解析':'Open Offline Analysis','进入格式转换':'Open Format Conversion','了解本站':'About This Site','暂未开放':'Not available yet','连接 PCAN、周立功或 Vector 设备，配置通道和波特率，安全地实时接收 CAN 报文。':'Connect PCAN, ZLG or Vector hardware, configure channels and bitrates, and safely receive CAN frames in real time.','加载 DBC 与 ASC 日志，选择信号、生成同步曲线，并进行报文统计和异常检查。':'Load DBC and ASC logs, select signals, generate synchronized charts, and inspect message statistics and anomalies.','在 ASC、LOG、TRC、BLF、TXT、MF4、MDF 之间转换，并导出 DBC 信号 CSV。':'Convert between ASC, LOG, TRC, BLF, TXT, MF4 and MDF, and export DBC signals to CSV.','面向充电通信场景的 GB/T 27930 报文识别、流程整理与异常辅助分析。':'Identify GB/T 27930 charging messages, organize communication flows and assist anomaly analysis.','面向 SAE J1939 网络的 PGN、SPN 与多包报文解析及通信状态分析。':'Analyze PGNs, SPNs, transport-protocol messages and communication state on SAE J1939 networks.','整理 CAN 总线、DBC、日志格式与常用工程工具的参考资源。':'Reference resources for CAN buses, DBC files, log formats and common engineering tools.','了解 CANAnalysis 的功能定位、本地数据处理原则以及后续维护计划。':'Learn about CANAnalysis, its local-data principles and future maintenance plans.','CANAnalysis · CAN 数据分析工作区':'CANAnalysis · CAN Data Workspace','CANAnalysis 首页':'CANAnalysis Home','CANAnalysis 提供在线 CAN 设备连接、离线报文解析、日志格式转换和 DBC 信号导出等工具。':'CANAnalysis provides live CAN hardware connections, offline message analysis, log conversion and DBC signal export.','根据实际需求选择在线连接、离线报文解析或日志格式转换。':'Choose online connection, offline message analysis or log conversion for your task.','在线连接 CAN 设备':'Connect to CAN Hardware','通过本机连接服务读取 CAN 总线，数据只在当前电脑与浏览器之间流转。':'Read the CAN bus through the local bridge. Data stays between this computer and browser.','本机连接服务':'Local Bridge','未检测':'Not detected','检测中…':'Checking…','检测服务':'Check Bridge','设备配置':'Device Configuration','设备型号':'Device Model','设备序号':'Device Index','通道号':'Channel','总线类型':'Bus Type','经典 CAN':'Classic CAN','CAN FD':'CAN FD','仲裁波特率':'Nominal Bitrate','数据波特率':'Data Bitrate','只接收（安全模式）':'Receive only (safe mode)','连接设备':'Connect Device','断开连接':'Disconnect','实时状态':'Live Status','等待连接本机服务':'Waiting for the local bridge','接收帧数':'Received Frames','错误数':'Errors','丢帧数':'Dropped Frames','实时报文':'Live Frames','清空列表':'Clear List','时间':'Time','CAN ID':'CAN ID','长度':'Length','数据':'Data','方向':'Direction','尚未收到报文':'No frames received','需要先在 Windows 电脑上安装并启动 CANAnalysis 本机连接服务。首版仅接收报文，不会向 CAN 总线发送数据。':'Install and start the CANAnalysis local bridge on Windows first. This release only receives frames and never transmits to the CAN bus.','连接服务已就绪':'Local bridge is ready','无法连接本机服务，请确认服务已经启动。':'Cannot reach the local bridge. Make sure it is running.','正在连接设备…':'Connecting to device…','设备已连接':'Device connected','设备已断开':'Device disconnected','连接失败':'Connection failed','刷新设备':'Refresh Devices','周立功 USBCAN-I':'ZLG USBCAN-I','周立功 USBCAN-II':'ZLG USBCAN-II','周立功 USBCANFD':'ZLG USBCANFD','CANAnalysis · 本地连接 · 数据不上传':'CANAnalysis · Local bridge · No data upload','页面还未完成开发，不急不急～':'This page is still under development.','发布':'Released','GitHub 提交号':'GitHub commit','Cloudflare 生产部署完成时间':'Cloudflare production deployment time','代码提交时间（部署记录不可用时的回退值）':'Commit time (fallback when deployment data is unavailable)','（站点版本清单）':'(site version manifest)'
  }));
